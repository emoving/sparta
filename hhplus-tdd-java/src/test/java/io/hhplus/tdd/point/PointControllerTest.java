...
class PointControllerTest {
    ...
    void point() throws Exception {
        ...
        when(pointService.getPoint(1L)).thenReturn(UserPointResponse.builder()
                .id(userPoint.id())
    ...
    void history() throws Exception {
        ...
        Mockito.when(pointService.getHistory(id)).thenReturn(PointHistoryListResponse.builder().pointHistoryList(mockHistory).build());
        ...
        mockMvc.perform(get("/point/{id}/histories", id))
                ...
                .andExpect(jsonPath("$.pointHistoryListResponse[0].id").value(1L))
                .andExpect(jsonPath("$.pointHistoryListResponse[0].userId").value(id))
                .andExpect(jsonPath("$.pointHistoryListResponse[0].amount").value(1000L))
                .andExpect(jsonPath("$.pointHistoryListResponse[0].type").value("CHARGE"))
                .andExpect(jsonPath("$.pointHistoryListResponse[1].id").value(2L))
                .andExpect(jsonPath("$.pointHistoryListResponse[1].userId").value(id))
                .andExpect(jsonPath("$.pointHistoryListResponse[1].amount").value(500L))
