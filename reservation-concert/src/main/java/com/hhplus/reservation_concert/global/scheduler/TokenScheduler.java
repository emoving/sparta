...
public class TokenScheduler {
    ...
    public void deleteExpiredTokens() {
        ...
        LocalDateTime expireThreshold = LocalDateTime.now().minusMinutes(EXPIRE_MINUTE);
        List<Token> activeTokens = tokenRepository.findByStatus(TokenStatus.ACTIVE);

        // EXPIRE_MINUTE 동안 미사용된 토큰 삭제
        for (Token token : activeTokens) {
            if (token.getRecentUsedAt().isBefore(expireThreshold)) {
